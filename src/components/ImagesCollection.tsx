...
export default async function ImagesCollection({ topic = "curated", page }: Props) {
  let url;
  if (topic === "curated" && page) {
    url = `https://api.pexels.com/v1/curated?page=${page}&per_page=50`;
  } else if (topic === "curated") {
    url = "https://api.pexels.com/v1/curated?per_page=50";
  } else if (!page) {
    // 1st page of search results
    url = `https://api.pexels.com/v1/search?query=${topic}&per_page=50`;
  } else {
    url = `https://api.pexels.com/v1/search?query=${topic}&page=${page}&per_page=50`;
  }
